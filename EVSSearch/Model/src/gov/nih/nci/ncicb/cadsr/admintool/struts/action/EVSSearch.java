...
public class EVSSearch  
{
  ...
  public static List searchEVS(String term
                              ,String  dtsVocab
                              ,String retired
                              , String sSearchInEVS
     ...
     else if(!term.equals("") && isMetaCodeSearch == false && !dtsVocab.equals("NCI Metathesaurus")) // Synonym search
    {
      ...
      try
      {
       if(retired.equals("Include") ) // do this if all concepts, including retired, should be included
        ...
        {    
        EVSQuery query = new EVSQueryImpl(); 
        query.getConceptWithPropertyMatching(dtsVocab,sSearchInEVS,term,10000);
        List concepts = null;
  ...
  && !term.equals("")  && codeFoundInThesaurus.equals(bFalse))
  {
      int length = 0;
      length = term.length();
      EVSQuery query = new EVSQueryImpl();
      ...
      query.searchByLoincId(term,sMetaSource);
        ...
        query.searchMetaThesaurus(term,sMetaLimit,sMetaSource, true, false, false);
      else if(!sSearchInEVS.equals("Concept Code"))  
        query.searchMetaThesaurus(term,sMetaLimit,sMetaSource, false, false, false);
      try
      {
        concepts = evsService.evsSearch(query);
      }
...
public static String getEVSCode(String prefName, String dtsVocab) 
{
  ...
  return CCode;
	...
	private  static String filterName(String nodeName, String type)
  {
